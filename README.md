...
| `psc_front_buttons.patch` | Ours (2026-09-20): the console's POWER button (`KEY_SLEEP`) is RetroArch's `"power"` key, so `input_exit_emulator = "power"` quits RetroArch and the launcher takes over; RESET is `KEY_PLAYPAUSE` = `"play"`, bound as `input_menu_toggle`. |
